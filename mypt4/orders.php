    ...
    <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post" class="form-horizontal">
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_staffs_a174807_pt4");
        $stmt->execute();
      ...
      <div class="form-group">
        <div class="col-sm-offset-3 col-sm-9">
          ...
          <?php } else { ?>
            <button class="btn btn-aml" type="submit" name="create"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span>Create</button>
          <?php } ?>
  ...
  <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
    ...
    <table class="table table-striped table-bordered">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a174807, tbl_staffs_a174807_pt4, tbl_customers_a174807_pt2 WHERE ";
        $sql = $sql."tbl_orders_a174807.fld_staff_num = tbl_staffs_a174807_pt4.fld_staff_num and ";
        $sql = $sql."tbl_orders_a174807.fld_customer_num = tbl_customers_a174807_pt2.fld_customer_num ORDER BY fld_order_date DESC";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a174807");
            $stmt->execute();
            $result = $stmt->fetchAll();
